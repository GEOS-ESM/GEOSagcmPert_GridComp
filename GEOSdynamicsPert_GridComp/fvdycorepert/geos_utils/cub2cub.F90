...
module CUB2CUB_mod
  ...
  public :: read_c2c_namelist,                                          &
            get_c2c_weight,                                             &
            get_c2c_weight_global,                                      &
            interpolate_c2c, do_c2c_interpolation
...
130       format (/,"output grid_spec file ",a," doesn't exist",/)
  ...
  subroutine init_cubsph_sgrid(npx, npy, ntiles, grid_file, super_grid)
    ...
    real(REAL8), dimension(3,2*(npx-1)+1,2*(npy-1)+1,ntiles), intent(out) :: super_grid
    !------------------------------------------------------------------!
    ! local variables                                                  !
    !------------------------------------------------------------------!
    real(REAL8) :: abs_vector
    real(REAL8), dimension(2,1:npx,1:npy,ntiles) :: sph_corner
    integer :: i, j, l, is, js
    !------------------------------------------------------------------!
  ...
  !====================================================================!
  subroutine get_c2c_weight(ntiles, npx_in, npy_in, &
                is_i, ie_i, js_i, je_i, isd_i, ied_i, jsd_i, jed_i, sph_in, &
                            npx_out, npy_out, is,ie, js,je, ls,le, sph_out, &
    ...
    integer, intent(in) :: is_i, ie_i, js_i, je_i
    integer, intent(in) :: isd_i, ied_i, jsd_i, jed_i
    integer, intent(in) :: is,ie, js,je, ls,le
    real(REAL8), intent(in) :: sph_in (2, is_i:ie_i+1, js_i:je_i+1, ls:le ),   &
                        sph_out(2, is:ie+1    , js:je+1    , ls:le )
    integer, intent(out) :: index_c2c(3, is:ie, js:je, ls:le)
    real(REAL8),    intent(out) :: weight_c2c (4, is:ie, js:je, ls:le)
    type(domain2d), intent(INOUT) :: domain_i
    !------------------------------------------------------------------!
    ! local variables                                                  !
    !------------------------------------------------------------------!
    real(REAL8) :: tmp(isd_i:ied_i, jsd_i:jed_i)
    real(REAL8) :: xyz_center_in (3, isd_i:ied_i, jsd_i:jed_i, ls:le),       &
            xyz_center_out(3, is   :ie   , js   :je   , ls:le)
    real(REAL8) :: abs_center, distance, shortest, sum, dist1, dist2, dist3, dist4
    integer :: i, j, l, ic, jc, i_guess, j_guess, lc, iter, index(3)
    logical :: found
    !------------------------------------------------------------------!
    ! calculate xyz cell corners and cell centers                      !
    !------------------------------------------------------------------!
    allocate(xyz_corner_in (3, is_i:ie_i+1, js_i:je_i+1, ls:le),      &
             xyz_corner_out(3, is:ie+1, js:je+1, ls:le))
    do l=ls,le
       do j=js_i,je_i+1
          do i=is_i,ie_i+1
    ...
    tmp = xyz_center_in(1,:,:,ls)
    call mpp_update_domains(tmp, domain_i)
    xyz_center_in(1,:,:,ls) = tmp
    tmp = xyz_center_in(2,:,:,ls)
    call mpp_update_domains(tmp, domain_i)
    xyz_center_in(2,:,:,ls) = tmp
    tmp = xyz_center_in(3,:,:,ls)
    call mpp_update_domains(tmp, domain_i)
    xyz_center_in(3,:,:,ls) = tmp
...
100             format ("FATAL: index search for ", 3i4," failed")
  ...
  contains
    ...
    subroutine set_closest_index(ig, jg, lg, ok)
      ...
      real(REAL8) :: angle_1, angle_1a, angle_1b,                              & 
...
#include "netcdf.inc"
    ...
    real(REAL8), intent(in) ::   weight_c2c (4, npx_out-1, npy_out-1, ntiles)
...
410         format("FATAL: vartype not equal nf_float: ", i3)
    ...
    subroutine write_var()
      ...
      real(REAL8), dimension(:,:,:,:), allocatable :: var_in, var_out
      real(REAL8), dimension(:,:,:), allocatable :: u, v ,ec1, ec2, ew1, ew2, es1, es2
      real(REAL8), dimension(:,:), allocatable :: dx, dy, dxa, dya, rdxa, rdya, cosa_s, sina_s
      real(REAL8), dimension(:), allocatable :: edge_vect_w, edge_vect_e, edge_vect_s, edge_vect_n
...
510         format("FATAL: vartype not equal nf_double: ", i3)
  ...
  subroutine do_c2c_interpolation(var_in, isd, ied, jsd, jed, nz, ntiles, &
    ...
    real(REAL8), dimension(isd:ied,jsd:jed,nz,ntiles), intent(in)  :: var_in
    real(REAL8), dimension(4,nx,ny,ntiles),            intent(in)  :: weight_c2c
    integer, dimension(3,nx,ny,ntiles),         intent(in)  :: index_c2c
    real(REAL8), dimension(nx,ny,nz,ntiles),           intent(out) :: var_out
    !------------------------------------------------------------------!
    ! local variables                                                  !
    !------------------------------------------------------------------!
  ...
  !====================================================================!
  subroutine get_c2c_weight_global(ntiles, npx_in, npy_in, sph_in,             &
    ...
    integer, intent(in) :: ntiles, npx_in, npy_in, npx_out, npy_out
    real(REAL8), intent(in) :: sph_in (2, 0:npx_in +1, 0:npy_in +1, ntiles),   &
                        sph_out(2, 0:npx_out+1, 0:npy_out+1, ntiles)
    integer, intent(out) :: index_c2c(3, npx_out-1, npy_out-1, ntiles)
    real(REAL8),    intent(out) :: weight_c2c (4, npx_out-1, npy_out-1, ntiles)
    !------------------------------------------------------------------!
    ! local variables                                                  !
    !------------------------------------------------------------------!
    real(REAL8) :: xyz_center_in (3, 0:npx_in  ,  0:npy_in  ,  ntiles),        &
...
100             format ("FATAL: index search for ", 3i4," failed")
  ...
  contains
    ...
    !------------------------------------------------------------------!
  end subroutine get_c2c_weight_global
  !====================================================================!
