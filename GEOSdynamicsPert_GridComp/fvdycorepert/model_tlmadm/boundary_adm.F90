...
module boundary_adm_mod
  ...
  public nested_grid_bc_apply_intt_adm
...
!  Differentiation of nested_grid_bc_apply_intt in reverse (adjoint) mode (with options split(a2b_edge_mod.a2b_ord4 a2b_edge_m
...
!   gradient     of useful results: var_nest
...
! to a flux BC on the tracers ONLY, which is implemented in fv_tracer.
  SUBROUTINE NESTED_GRID_BC_APPLY_INTT_ADM(var_nest, var_nest_ad, istag&
...
&   INTENT(INOUT) :: var_nest_ad
    ...
    IF (je .EQ. npy - 1) THEN
      ...
      END IF
      DO k=npz,1,-1
        DO j=jed+jstag,npy+jstag,-1
          DO i=iend+istag,istart,-1
            var_nest_ad(i, j, k) = 0.0
          END DO
        END DO
      END DO
    END IF
    ...
    IF (branch .EQ. 0) THEN
      DO k=npz,1,-1
        DO j=jed+jstag,jsd,-1
          DO i=ied+istag,npx+istag,-1
            var_nest_ad(i, j, k) = 0.0
          END DO
        END DO
      END DO
    ...
    IF (branch .EQ. 0) THEN
      ...
      END DO
    END IF
  END SUBROUTINE NESTED_GRID_BC_APPLY_INTT_ADM
! A NOTE ON BCTYPE: currently only an interpolation BC is implemented,
! bctype >= 2 currently correspond
! to a flux BC on the tracers ONLY, which is implemented in fv_tracer.
